...
>> Given that Elvis is a twin ("T"), what is the probability he was an identical twin ("I")? Bayes Thm: p(I|T)=p(I)p(T|I)/p(T)=(1/300)(1)/(1/300+1/125)=.294
...
>> Frequentist statistics is the interpretation of probability where the probability represents the frequency of an occurence; an experiment can be repeated infinite times and each time the results are independent.  The Bayesian probability is more "iterative" in nature; an initial probability is specified for a hypothesis, then the hypothesis is updated with the new data.
